...
class AllOrdersCntroller extends Controller
{
    ...
    public function create()
    {
        ...
        $chanels = Channel::all();
        return view('admin.pages.allorders.create', compact('allsrors','chanels','allshipin'));
    }
...
//        var_dump($request->all(), $request->total_price_products);
//        exit;
...
//            'num_products' => $request->num_products,
            ...
            'products_details' => serialize($request->products), // un serialize and loop when fetching data
...
//        }
    ...
    public function getProductInfo(int $id) {
        $product = DB::table('stores')
            ->select('id', 'product_name', 'description', 'selling_price', 'buy_price','total_amount')
            ->where('id', $id)->first();
